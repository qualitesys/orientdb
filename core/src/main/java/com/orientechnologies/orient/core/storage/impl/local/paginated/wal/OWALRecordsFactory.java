...
public final class OWALRecordsFactory {
  ...
  private static final int RECORD_ID_SIZE = OShortSerializer.SHORT_SIZE;
  private static final int OPERATION_ID_SIZE = OIntegerSerializer.INT_SIZE;
  private static final int UN_COMPRESSED_CONTENT_SIZE = OIntegerSerializer.INT_SIZE;

  private static final int RECORD_ID_OFFSET = 0;
  private static final int OPERATION_ID_OFFSET = RECORD_ID_OFFSET + RECORD_ID_SIZE;
  private static final int UN_COMPRESSED_CONTENT_SIZE_OFFSET =
      OPERATION_ID_OFFSET + OPERATION_ID_SIZE;

  private static final int METADATA_SIZE = RECORD_ID_SIZE + OPERATION_ID_SIZE;
  private static final int COMPRESSED_METADATA_SIZE = METADATA_SIZE + UN_COMPRESSED_CONTENT_SIZE;

  public static ByteBuffer toStream(final WriteableWALRecord walRecord) {
    // record content size + record id + operation id
    ...
    // leave space for the operation id
    content.position(content.position() + OPERATION_ID_SIZE);
    ...
    final int maxCompressedLength = compressor.maxCompressedLength(contentSize - METADATA_SIZE);
    ...
    if (compressedLength + COMPRESSED_METADATA_SIZE < contentSize) {
      ...
      compressedContent.putInt(UN_COMPRESSED_CONTENT_SIZE_OFFSET, contentSize);
  ...
  public WriteableWALRecord fromStream(byte[] content) {
    ...
    if (recordId < 0) {
      final int originalLen = OIntegerSerializer.INSTANCE
          .deserializeNative(content, UN_COMPRESSED_CONTENT_SIZE_OFFSET);
  ...
  public static void serializeOperationId(final ByteBuffer buffer, final int operationId) {
